...
//        };
    ...
    as.controller('AlbumListCtrl', function($scope, $rootScope, $http, $location) {
        var load = function() {
            ...
            $http.get($rootScope.appUrl + '/album-rest')
        ...
        $scope.delAlbum = function(index) {
            ...
            $http
                    .delete($rootScope.appUrl + '/album-rest/' + todel.id)
                    .success(function(data, status, headers, config) {
                        load();
    ...
    as.controller('EditAlbumCtrl', function($scope, $rootScope, $http, $routeParams, $location) {
        ...
        var load = function() {
            console.log('call load()...');
            $http.get($rootScope.appUrl + '/album-rest/' + $routeParams['id'])
                    .success(function(data, status, headers, config) {
        ...
        $scope.updateAlbum = function() {
            ...
            $http.put($rootScope.appUrl + '/album-rest/' + $scope.album.id, $scope.album)
                    .success(function(data, status, headers, config) {
